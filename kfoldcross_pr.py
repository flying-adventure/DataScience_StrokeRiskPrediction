...
def find_best_k_and_evaluate(df: pd.DataFrame, k_values=range(5, 11)):
    ...
    for k in k_values:
        ...
        fold_pr_auc_scores = [] # PR AUC는 Precision-Recall 곡선 아래의 면적을 나타내는 지표임

        # PR AUC는 모델이 양성 클래스(Positive class)를 얼마나 잘 식별하는지 나타냄
        # PR AUC = 0.90 → 양성 클래스 판별 매우 잘함
        ...
        for train_idx, test_idx in skf.split(X, y):
            ...
            pr_auc = average_precision_score(y_test, y_prob)  # 평가지표 변경
        ...
        all_scores[k] = mean_pr_auc
        print(f"K={k} 평균 PR AUC: {mean_pr_auc:.4f}")

        if mean_pr_auc > best_score:
            best_score = mean_pr_auc
            best_k = k

    print(f"\n✅ 가장 좋은 K는 {best_k}이며, 평균 PR AUC는 {best_score:.4f}입니다.")
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        final_pr_auc_scores.append(pr_auc)
    ...
    print(f"\n최종 평균 PR AUC (K={best_k}): {np.mean(final_pr_auc_scores):.4f}")
